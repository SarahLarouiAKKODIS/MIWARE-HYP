...
def detect_micas_bd2200_clean(
    ...
    *,
    ...
    """
    Détection micas sur image déjà prétraitée.
    ...
    score :
      ...
      - avec BD1900 : sqrt(norm(BD2200) * (1 - norm(BD1900)))
    """
    ...
    with rasterio.open(tif_path) as src:
        ...
        if has_bd1900 and bd1900 is not None:
            s1900_bad = normalize01(bd1900, bd1900_score_min, bd1900_score_max)
            s1900 = 1.0 - s1900_bad
    ...
    if verbose:
        ...
        if prob_stats is not None:
            print(
                "Probabilité sur pixels détectés (min/mean/max) :",
                prob_stats["min"],
                prob_stats["mean"],
                prob_stats["max"],
            )
    ...
    return {
        "outputs": {
            "bd2200": out_bd2200,
            "bd1900": out_bd1900 if (has_bd1900 and bd1900 is not None) else None,
            "mask": out_mask,
            "prob": out_prob,
        },

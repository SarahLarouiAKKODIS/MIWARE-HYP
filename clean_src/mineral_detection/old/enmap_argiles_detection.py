...
def detect_argiles_bd2200(
    ...
    *,
    ...
    write_outputs: bool = False,   # ton script avait l’écriture commentée -> default False
    compress: str = "lzw",
    verbose: bool = True,
) -> dict:
    """
    Détection argiles via band depth ~2200 nm (Al-OH) + optionnel contrôle ~1900 nm (H2O/OH).
    ...
    Écrit (si write_outputs=True) :
      - BD2200_argiles.tif (float32)
      - BD1900_control.tif (float32, si calculé)
      - argiles_mask.tif (uint8: 0/255)
      - argiles_probability.tif (float32)

    Returns
    -------
    dict avec outputs, bandes, params, stats, + arrays (bd/prob/mask) si tu veux les récupérer.
    """
